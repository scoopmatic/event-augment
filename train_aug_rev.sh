...
python OpenNMT-py/translate.py -seed 10101 -gpu 0 -model $model/rev_model_step_$i.pt -src $data/devel.output.pcs -output pred.txt.pcs -replace_unk -max_length 80 -batch_size $BS
#python event-augment/sentpiece/p2s.py pred.txt.pcs event-augment/sentpiece/m.model > pred.txt
cat test_pred.txt.pcs|python piecemix2text.py > pred.txt
#cat pred.txt | sed "s/ – /–/g" |sed "s/ - /-/g" | sed "s/ — /—/g" | sed "s/( /(/g" | sed "s/ )/)/g" > pred.txt.detok
BLEU=$(perl OpenNMT-py/tools/multi-bleu.perl $data/devel.output.detok < pred.txt)
echo $BLEU $i >> rev_eval.txt
echo $BLEU
done

BEST="$model/rev_model_step_$(cat eval.txt | cut -d" " -f3,9|sort -n|tail -1|cut -d" " -f2).pt"
...
python OpenNMT-py/translate.py -seed 10101 -gpu 0 -model $BEST -src $data/test.output.pcs -output test_pred.txt.pcs -replace_unk -max_length 80 -batch_size $BS
#python event-augment/sentpiece/p2s.py test_pred.txt.pcs event-augment/sentpiece/m.model > test_pred.txt
cat test_pred.txt.pcs|python piecemix2text.py > test_pred.txt
